...
class WormHoleClient:
    ...
    async def _handle_message(
        self, ws: aiohttp.ClientWebSocketResponse, data: dict[str, object]
    ) -> None:
        """Handle messages from the server."""
        msg_type = data.get("type")

        if msg_type == "connected":
            self.tunnel_id = str(data.get("tunnel_id", ""))
            ...
            await ws.send_json(
                {
                    ...
                    "status": 502,
                    "headers": {},
                    "body": f"Error forwarding request: {e!s}",
                }
            )


async def run_local_test_server(port: int = 3000) -> None:
